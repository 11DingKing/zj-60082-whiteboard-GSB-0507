...
import { detectSnapLines, getSnapAdjustment } from "../canvas/snap";
import {
  Shape,
  Point,
  ToolType,
...
const Canvas: React.FC<CanvasProps> = ({ onTextEdit }) => {
  ...
  const { offsetX, offsetY, zoom, showGrid, panBy, zoomTo, setShowGrid } =
    useCanvasStore();
  const {
    currentTool,
    fillColor,
  ...
  const isPointOnRotateHandle = useCallback(
    (canvasPoint: Point): boolean => {
      const selectedShapes = getShapes(selectedShapeIds);
  ...
  const getShapeAtPoint = useCallback(
    (canvasPoint: Point): Shape | undefined => {
  ...
  const createShape = useCallback(
    (tool: ToolType, startPoint: Point, endPoint: Point): Shape => {
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (
        mouseState.isMiddleDown ||
        (mouseState.spacePressed && e.buttons === 1)
      ) {
      ...
      if (isDragging && selectedShapeIds.length > 0) {
        ...
        selectedShapes.forEach((shape) => {
          const offset = dragOffsets.get(shape.id);
          if (offset) {
            const newX = canvasPoint.x + offset.x;
            const newY = canvasPoint.y + offset.y;
            dragMinX = Math.min(dragMinX, newX);
            dragMaxX = Math.max(dragMaxX, newX + shape.width);
            dragMinY = Math.min(dragMinY, newY);
            dragMaxY = Math.max(dragMaxY, newY + shape.height);
          }
        });

        const tempShapes = selectedShapes.map((shape) => {
          const offset = dragOffsets.get(shape.id);
          if (offset) {
            return {
              ...shape,
        ...
        const snapLines = detectSnapLines(tempShapes, otherShapes, {
          minX: dragMinX,
          maxX: dragMaxX,
          minY: dragMinY,
          maxY: dragMaxY,
        ...
        const { dx, dy } = getSnapAdjustment(snapLines);

        const guides = snapLines.map((line) => ({
          type: line.type,
        ...
        selectedShapeIds.forEach((id) => {
          ...
          if (offset) {
            let newX = canvasPoint.x + offset.x + dx;
            let newY = canvasPoint.y + offset.y + dy;
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (ctrlOrMeta && e.key === "a") {
        e.preventDefault();
        const visibleUnlockedShapes = shapes.filter(
          (s) => s.visible && !s.locked,
        );
  ...
  return (
    <div ref={containerRef} className="canvas-wrapper">
      <canvas
        ...
        style={{
          cursor: mouseState.spacePressed
            ? "grab"
            : isDragging
              ? "grabbing"
              : currentTool === "select"
      ...
      {smartGuides.map((guide, index) => {
        const screenPos =
          guide.type === "horizontal"
            ? canvasToScreen(0, guide.position).y
            : canvasToScreen(guide.position, 0).x;

        const classNames = ["snap-guide", guide.type, guide.category || "edge"]
          .filter(Boolean)
          .join(" ");
