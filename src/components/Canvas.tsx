...
import { detectSnapLines, SnapLine } from "../canvas/snap";
import {
  Shape,
  Point,
  ToolType,
...
const Canvas: React.FC<CanvasProps> = ({ onTextEdit }) => {
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (isDragging && selectedShapeIds.length > 0) {
        ...
        const updatedMovingShapes = useProjectStore
          .getState()
          .shapes.filter((s: Shape) => selectedShapeIds.includes(s.id));
        const allCurrentShapes = useProjectStore.getState().shapes;
        const canvasW = containerRef.current?.clientWidth || 800;
        const canvasH = containerRef.current?.clientHeight || 600;
        const snapResult = detectSnapLines(
          updatedMovingShapes,
          allCurrentShapes,
          canvasW,
          canvasH,
          ...
          if (snapResult.snapX !== null || snapResult.snapY !== null) {
            selectedShapeIds.forEach((id) => {
              const shape = useProjectStore
                .getState()
                .shapes.find((s: Shape) => s.id === id);
              if (shape) {
                const updates: Partial<Shape> = {};
                if (snapResult.snapX !== null)
                  updates.x = shape.x + snapResult.snapX;
                if (snapResult.snapY !== null)
                  updates.y = shape.y + snapResult.snapY;
  ...
  return (
    <div ref={containerRef} className="canvas-wrapper">
      ...
      {snapLines.length > 0 && (
        <div className="snap-lines-overlay">
          {snapLines.map((line, idx) => {
            if (line.type === "horizontal") {
              const screenY = line.position * zoom + offsetY;
              return (
                <div
                  key={`snap-${idx}`}
                  className="snap-line snap-line-horizontal snap-line-fadein"
                  style={{ top: screenY }}
                />
              );
            } else {
              const screenX = line.position * zoom + offsetX;
              return (
                <div
                  key={`snap-${idx}`}
                  className="snap-line snap-line-vertical snap-line-fadein"
                  style={{ left: screenX }}
                />
              );
            }
